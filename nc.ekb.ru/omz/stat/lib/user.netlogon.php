...
<TR Class='tHeader'><TH>День</TH><TH>Время</TH><TH>IP</TH><TH>Компьютер</TH><TH title='Имя хоста-клиента RDP'>RDP</TH><TH>Версия Windows</TH>
<? if(2==$CFG->Who): ?>
...
while($r=mysql_fetch_object($q)):
 echo "<TR><TD Align='Right'>", $r->Day,
    ...
    "<BR /></TD><TD>", '<A Title="Open in VNC" hRef="vnc:', $r->IP, '">&raquo;</A>', $r->IP,
//    "<BR /></TD><TD>", '<A hRef="file://///',$r->Computer,'">&raquo;</A>', htmlspecialchars($r->Computer),
    "<BR /></TD><TD>", ($r->Domain?"<code>".strtoupper($r->Domain{0})."\\</code>":""), htmlspecialchars($r->Computer),
    "<BR /></TD><TD>", htmlspecialchars($r->RDP),
    "<BR /></TD><TD><Small>", htmlspecialchars($r->winVer), "</Small>";
 if(2==$CFG->Who)
    echo

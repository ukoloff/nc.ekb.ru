...
{
 global $CFG;
 $CFG->ldapError='Ошибка '.ldap_errno($CFG->h).': '.ldap_error($CFG->h);
}
...
// echo "<!--\nSave"; print_r($save); echo "\nDel"; print_r($del); echo "\nMod"; print_r($mod); echo "\n-->";
 if(count($del)>0 and !ldap_mod_del($CFG->h, $dn, $del)):
  ldapError();
  return false;
 endif;
 if(count($mod)>0 and !ldap_modify($CFG->h, $dn, $mod)):
  ldapError();
  return false;
 endif;
 return true;
...
function ldapCreate($objRename, $attrs)
{
 ...
 if(ldap_add($CFG->h, $objRename->dn, $add)) return true;
 ldapError();
 return false;
}
...
function ldapRename($objRename)
{
 ...
 if(ldap_rename($CFG->h, $objRename->dn, $objRename->rdn->str(), $objRename->dnIn->str(), true)) return true;
 ldapError();
 return false;
}
...
function ldapDelete($dn)
{
 global $CFG;
 if(!$dn) return;
 if(ldap_delete($CFG->h, $dn)) return true;
 ldapError();
 return false;
}
...
function ldapChangePass($u, $pass)
{
 ...
 ldap_modify($CFG->h, user2dn($u), Array('unicodePwd'=>pass2unicode(randomPass())));
 
 if(!ldap_modify($CFG->h, user2dn($u), Array('unicodePwd'=>pass2unicode($pass)))):
  ldapError();
  return false;
 endif;
 onChangePass($u);
...
// mysql_query("Insert Into Passwords(u, Who, ip) Values('$u', '$Who', '$ip')");
 mysql_query("Insert Into uxmJournal.Password(u, Who, ip, Dom) Values('$u', '$Who', '$ip', 'l')");
}

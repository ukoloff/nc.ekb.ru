...
Class smbClient
{
 ...
 {	# Почему работает такой quoting я так и не понял, но он работает
  ...
  $cmd="/usr/bin/smbclient ".escapeShellArg('//'.$this->Server.'/'.$this->Folder).
    " -U ".escapeShellArg($_SERVER['PHP_AUTH_USER'].'%'.$_SERVER['PHP_AUTH_PW'])." -c ".escapeShellArg(utf8($cmd));
 ...
 function getFile($smbPath, $localPath)
 {
 ...
 function putFile($localPath, $smbPath, $dirs)
 {
  ...
  $this->runSmbClient('put', $localPath, $this->winPath($smbPath));
 }
}
